...
from config import reports_db, uploads, downloads, taxreport_base
from fifocalc import *
...
def step1():
    ...
    return render_template('step1.html', list=fileList)
...
def step3():
    ...
    #coming from step 2
    if request.form.get('fifo') is not None:
        method = request.form.get('FIFO_selected')

    if request.form.get('lifo') is not None:
        method = request.form.get('LIFO_selected')
    ...
    save_taxinfo(session, taxyear, method=method)

    query = session.query(Transactions).all()
    taxes = session.query(Taxes).all()
    fileList = os.listdir(uploads)
    return render_template('step3.html', list=fileList,taxoptions=taxoptions, taxyears=taxyearoptions,
                           trans=query, taxes=taxes)
...
def save_taxinfo(session, taxyear, method='FIFO'):
    ...
    taxobj.Short_Term_Gains, taxobj.Short_Term_Tax, \
    taxobj.Long_Term_Gains, taxobj.Long_Term_Tax = calculate_gl_estimatedtaxes(session, current_tax_year=taxyear,
                                                                               method=method)
...
def send_file2():
    try:
        f = request.files.getlist('file2upload')
        for current in f:
            if current.filename != '':
                current.save(os.path.join(uploads, secure_filename(current.filename)))
    except Exception as e:
